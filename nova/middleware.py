...
class LoggingMiddleware:
    ...
    @staticmethod
    def copy_request_body(request):
        request.custom_body = copy.deepcopy(request.body)
        return request

    def before_view(self, request: WSGIRequest) -> WSGIRequest:
        """Обработка запроса перед view."""
        request = self.copy_request_body(request)
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            if request.FILES:
                ...
                for name, file in request.FILES.items():
                    request_body[name] = file.name

                return request_body
            try:
                return json.loads(request.custom_body.decode("utf-8"))
